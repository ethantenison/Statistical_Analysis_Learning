...
logdf <- select(logdf, -c(irrigation_residential, postal_code))
...
set.seed(88)
dfknn <- select(dfknn, -c(irrigation_residential, postal_code))
...
train_labels = train[["HiResI
                      rr"]]
test_knn = subset(dfknn, split == FALSE)
test_labels = test[["HiResIrr"]]
...
The overall test error is $0.1540418$.The Sensitivity of the LDA function is low at $0.3017945$, but the specificity is high at $0.96$.  

```{r LDA}
library(MASS)

fit <- lda(HiResIrr ~ year + irrigation_multi_family + multi_family + residential, data=train, CV=TRUE)

ct <- table(train$HiResIrr, fit$class)
...
The test error for the QDA function was slightly higher than LDA at $0.1599774$
```{r QDA}
fit <- qda(HiResIrr ~ year + irrigation_multi_family + multi_family + residential, data=train, CV=TRUE)
...
fit <- lda(class ~ x1 + x2, data=df, CV=TRUE)

ct <- table(df$class, fit$class)
diag(prop.table(ct, 1))
sum(diag(prop.table(ct)))

fitGraph <- ggplotLDAPrep(fit)
ggplot(data=fitgraph, aes(x=x1,y=x2,col=class, shape=class)) + 
       geom_point() 
```